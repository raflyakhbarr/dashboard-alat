...
async function submitPenindaklanjut(formData: FormData) {
  ...
  if (!session || (session.role !== 'fasilitas' && session.role !== 'peralatan_terminal' && session.role !== 'perencanaan_persediaan')) {
    redirect('/login');
  }
  ...
  try {
    ...
    redirect(`/${session.role}/penindak-lanjut?success=true`);
  } catch (error: any) {
    console.error('Error creating penindaklanjut:', error);
    redirect(`/${session.role}/penindak-lanjut?error=true`);
  }
}
...
export default async function PenindakLanjutPage({
  ...
  const roleMapping: Record<string, 'fasilitas' | 'peralatan_terminal' | 'perencanaan_persediaan'> = {
    fasilitas: 'fasilitas',
    peralatan_terminal: 'peralatan_terminal',
    perencanaan_persediaan: 'perencanaan_persediaan',
  };

  const penindakLanjutRole = roleMapping[session.role];

  // Get all pending reports for this role
  const laporanList = await getLaporanByPenindakLanjut(penindakLanjutRole);
            ...
            <>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Daftar laporan kerusakan yang ditugaskan ke tim Anda
                  </CardDescription>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="space-y-4">
                      {pendingReports.map((laporan) => (
                        ...
                        >
                          ...
                          <Link href={`/${session.role}/penindak-lanjut?laporan_id=${laporan.id}`}>
                            <Button>
                ...
                <CardContent>
                  ...
                  <form action={submitPenindaklanjut} className="space-y-6">
                    ...
                    <div className="flex gap-4">
                      ...
                      <Link href={`/${session.role}/penindak-lanjut`}>
